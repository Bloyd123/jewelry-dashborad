...
export const AdditionalDetailsForm: React.FC<AdditionalDetailsFormProps> = ({
  ...
  const [details, setDetails] = useState<AdditionalDetailsFormData>({
    ...
    notes: initialData?.notes || 'Rates updated based on international market prices',
    internalNotes: initialData?.internalNotes || 'Market volatility high due to festival season',
  });

  // Handle field change
  const handleFieldChange = (field: keyof AdditionalDetailsFormData, value: string) => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="weight-unit">
              {t('metalRates.weightUnit')}
            </Label>
            <Select
              value={details.weightUnit}
              onChange={(value) => handleFieldChange('weightUnit', value as WeightUnit)}
          ...
          <div className="space-y-2">
            <Label htmlFor="rate-source">
              {t('metalRates.rateSource')}
            </Label>
            <Select
              value={details.rateSource}
              onChange={(value) => handleFieldChange('rateSource', value as RateSource)}
