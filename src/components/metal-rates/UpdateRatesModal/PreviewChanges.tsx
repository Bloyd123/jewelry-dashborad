...
export const PreviewChanges: React.FC<PreviewChangesProps> = ({
  ...
  return (
    <div className={cn('space-y-3', className)}>
      ...
      {/* Changes List */}
      <div className="rounded-lg border border-border-primary bg-bg-secondary divide-y divide-border-primary">
          ...
          return (
            <div key={index} className="p-4">
              <div className="flex items-center justify-between gap-4">
                ...
                <div
                  className={cn(
                    'flex items-center gap-2 min-w-[140px] justify-end',
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div className="rounded-lg border border-border-primary bg-bg-secondary p-3">
          ...
          <div className="text-lg font-semibold text-status-success">
            {
              changes.filter((c) => getChangeDetails(c.oldRate, c.newRate).trend === 'up')
                .length
            }
          </div>
        </div>
        ...
        <div className="rounded-lg border border-border-primary bg-bg-secondary p-3">
          ...
          <div className="text-lg font-semibold text-status-error">
            {
              changes.filter((c) => getChangeDetails(c.oldRate, c.newRate).trend === 'down')
                .length
            }
          </div>
        </div>
        ...
        <div className="rounded-lg border border-border-primary bg-bg-secondary p-3">
          ...
          <div className="text-lg font-semibold text-text-tertiary">
            {
              changes.filter((c) => getChangeDetails(c.oldRate, c.newRate).trend === 'stable')
                .length
            }
          </div>
        </div>
