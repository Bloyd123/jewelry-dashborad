...
export const updateShop = createAsyncThunk(
  'shop/updateShop',
  async ({ shopId, updates }: { shopId: string; updates: UpdateShopRequest }, { rejectWithValue }) => {
    try {
...
export const updateShopSettings = createAsyncThunk(
  ...
  async ({ shopId, settings }: { shopId: string; settings: UpdateShopSettingsRequest }, { rejectWithValue }) => {
    try {
...
export const updateMetalRates = createAsyncThunk(
  ...
  async ({ shopId, rates }: { shopId: string; rates: UpdateMetalRatesRequest }, { rejectWithValue }) => {
    try {
      const response = await authService.updateMetalRates(shopId, rates);
      return response.data;
    } catch (error: any) {
      return rejectWithValue(error.response?.data?.message || 'Failed to update metal rates');
    }
  }
);
